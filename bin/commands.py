...
def offer(ice, call_id, from_tag, label, sdpaddr, port):
...
def answer(ice, call_id, from_tag, to_tag, label, sdpaddr, port):
    # TODO: Somehow pass an argument that contains all the unnecessary keys 
    # and only write it to a string if it is defined, otherwise not.
    return fr'''
    {{
...
def query(call_id, from_tag = '-1', to_tag = '-1'):
...
def start_recording(call_id, from_tag = '-1', to_tag = '-1', via_branch = '-1'):
...
def block_dtmf(call_id, from_tag = '-1', address = '-1', label = '-1'):
...
def unblock_dtmf(call_id, all = False):
    ''' Unblock DTMF. 
...
def block_media(call_id, from_tag = '-1', address = '-1', label = '-1'):
    ''' Block media packets. 
...
def unblock_media(call_id, all = False):
...
def start_forwarding(call_id, from_tag = '-1', address = '-1', label = '-1'):
...
def stop_forwarding(call_id, all = False):
    ''' Stop forwarding.
...
def play_media(call_id, file, from_tag = '-1', address = '-1', label = '-1',
                all = False):
...
def stop_media(call_id, from_tag = '-1', address = '-1', label = '-1',
                all = False):
    ''' Stop playback. 
...
def play_dtmf(call_id, file, code, from_tag = '-1', address = '-1', 
            label = '-1', all = False):
